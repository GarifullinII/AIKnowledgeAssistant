...
- `app/services/rag_service.py` строит embedding для вопроса, ищет наиболее похожие чанки, использует кеш и генерирует ответ на основе найденного контекста;
...
- `app/core/config.py` теперь содержит настройку `upload_dir`, OpenAI-настройки, а также параметры `PostgreSQL` и `Redis`.

Что реально происходит при вызове endpoint:

- `GET /api/health` возвращает `{"status": "ok"}`;
- `POST /api/upload` принимает файл, проверяет имя, тип или расширение, сохраняет его в каталог `data/uploads`, извлекает текст, разбивает его на чанки, строит embeddings, сохраняет документ и чанки в `PostgreSQL` и возвращает метаданные документа;
...
- `POST /api/ask` принимает вопрос и необязательный `document_id`, сначала проверяет кеш в `Redis`, затем при необходимости строит embedding вопроса, находит похожие чанки, отбирает контекст и генерирует ответ через LLM.
...
- endpoint `/api/ask` уже использует retrieval по чанкам, кеширование в `Redis` и LLM generation, но качество и надежность пайплайна еще требуют доработки;
